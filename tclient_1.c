...
{
    int sock_father = 0,sock_child = 0;
    fd_set rfds;
    ...
    while(1){
        ...
        pthread_mutex_lock(&argThreadClock.flag_mtx);
        if(argThreadClock.flag == 1){
            argThreadClock.flag = 0;
            pthread_mutex_unlock(&argThreadClock.flag_mtx);
            child_protocol(sock_father,sock_child,&argThreadClock);
        }else{
...
void child_protocol(int sock_server,int sock_child,ThreadArg_t *thr_arg){
    Msg pkg_req,pkg_sync1_father,pkg_sync2_father;
    ...
    setRequest(&pkg_req);
    sendMsg(sock_server,&pkg_req);

    err = recvMsg(sock_server,&pkg_sync1_father);
    if (err == -1){
        perror("ERROR recv msg");
        return;
    }else if(err == 0){
    ...
    if(getType(&pkg_sync1_father.hdr) == TYPE_SYN_1){
        ...
        t1 = thr_arg->time_counter;
        pthread_mutex_unlock(&thr_arg->counter_mtx);

        pthread_mutex_lock(&thr_arg->wait_sync_req_mtx);
        pthread_cond_wait(&thr_arg->cond_wait_sync_req,&thr_arg->wait_sync_req_mtx);
        pthread_mutex_unlock(&thr_arg->wait_sync_req_mtx);

        err = recvMsg(sock_server,&pkg_sync2_father);
        if (err == -1){
            perror("ERROR recv msg");
            return;
        }else if(err == 0){
        ...
        if(getType(&pkg_sync2_father.hdr) == TYPE_SYN_2){
            ...
            pthread_mutex_lock(&thr_arg->counter_mtx);
            t2 = thr_arg->time_counter;
            thr_arg->time_counter = (unsigned) (t2 - t1 + t0 - SIMU_TIME * MICRO_SECONDS);
            pthread_mutex_unlock(&thr_arg->counter_mtx);
...
void *threadClock_client(void *thr_arg){
    ...
    // while(getType(&pkg_sync.hdr) != TYPE_SYN_CLOCK){
        recvMsg(thread_arg->sock_father,&pkg_sync);
        printf("Type %d \n",getType(&pkg_sync.hdr));
    // }
    // while(getType(&pkg_sync.hdr) != TYPE_SYN_CLOCK);
    // printf("sending for pkg\n");
    setSynClock(&pkg_sync_2);
    sendMsg(thread_arg->sock_child,&pkg_sync_2);
    // sendMsg(thread_arg->sock_father,&pkg_sync);
    ...
    while(global_count < NUM_ITER_SIM + 1){
            ...
            // for(int i = 0; i < AUX_VAL; i++){
                ...
                if(i == (int) SECONDS_FOR_DRIFT/SIMU_TIME - 1){
                // if(i == (int) AUX_VAL - 1){
                    pthread_mutex_lock(&thread_arg->flag_mtx);
                    thread_arg->flag = 1;
                    pthread_mutex_unlock(&thread_arg->flag_mtx);
                }
