...
reader = csv.reader(open("data_6/server.csv"), delimiter=",")
...
reader = csv.reader(open("data_6/client_end.csv"), delimiter=",")
...
for row in reader:
    if(flag == 0):
        ...
        first_item = int(row[1])
        flag = 2
    else:
        if(int(row[1]) > first_item and int(row[1]) < last_item):
            print(row)        
...
reader = csv.reader(open("data_6/client_1.csv"), delimiter=",")
# reader = csv.reader(open("client_end.csv"), delimiter=",")
aux = []
i=0
flag = 0
...
reader = csv.reader(open("data_6/client_2.csv"), delimiter=",")
# reader = csv.reader(open("client_end.csv"), delimiter=",")
aux = []
i=0
flag = 0
for row in reader:
    if(flag == 0):
        ...
        if(int(row[1]) > first_item and int(row[1]) < last_item):
            ...
            aux.append(int(row[3]))
            i = i + 1
client_2 = np.asarray(aux)
print(len(client_2))

#%%


reader = csv.reader(open("data_6/client_3.csv"), delimiter=",")
# reader = csv.reader(open("client_end.csv"), delimiter=",")
aux = []
i=0
flag = 0
...
lim_inf_1 = 7
lim_sup_1 = samples - lim_inf_server
lim_inf_end_1 = 0
...
ax.scatter(client_end_index[lim_inf_end_1:lim_sup_end_1],client_end_time[lim_inf_end_1:lim_sup_end_1],color = 'r')
...
# dif_2 = client_end_time[lim_inf_end_1:lim_sup_end_1] - server_time[lim_inf_server:lim_sup_server_for_end_1]
